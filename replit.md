...
*   **Opportunity Analysis & Reporting:** AI generates opportunity scores, market size estimates, and business model suggestions. A Consultant Studio offers a four-path validation system, and a report tracking system logs usage.
    ...
    - **Clone Success Guest Payment Flow:** Non-authenticated users can purchase Clone Success analysis ($49) via Stripe Checkout. Free view shows map with location names and match scores only (demographics, competition, population, income, key factors are blurred). Payment flow: guest enters email, redirects to Stripe Checkout, returns with session_id for server-side verification before unlocking. Endpoints: `POST /api/v1/payments/create-clone-checkout`, `GET /api/v1/payments/verify-clone-checkout`.
