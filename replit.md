...
*   **Enhanced Research Workspace:** Extends the base workspace via FK (opt-in, doesn't replace). Features 3 predefined workflow templates (Lean Validation, Enterprise B2B, Product-Market Fit) with stages/tasks, plus custom workflow support. Includes 8 specialized tabs: Overview (summary + AI recommendations), Research Hub (artifact management), Workflow (stage/task tracking), Interviews (AI-generated guides), Surveys (AI-generated questions), Competitors (AI analysis framework), Financial (AI financial modeling), and Analytics (completion metrics). AI insights currently use deterministic generated content. Backend: `enhanced_workspace.py` (models), `enhanced_workspace_service.py`, `enhanced_workspaces.py` (router). Frontend: 8 tab components in `frontend/src/components/workspace/`. Tables created via `Base.metadata.create_all(checkfirst=True)` on startup.
