...
*   **API Access:** Business Track teams can enable API access with rate-limited public endpoints authenticated via API keys. Features include API key generation/revocation, usage tracking, and scoped permissions (opportunities:read). Endpoints: `GET/POST /api/teams/{team_id}/api-keys`, `DELETE /api/teams/{team_id}/api-keys/{key_id}`, `POST /api/teams/{team_id}/api-access/enable`. Public API at `/api/v1/public/*` for external integrations.
*   **Team Collaboration:** Shared opportunity access within teams with priority/status tracking, team notes/comments on opportunities, and activity feed for team actions. Endpoints: `POST/GET /api/teams/{team_id}/opportunities`, `PATCH/DELETE /api/teams/{team_id}/opportunities/{id}`, `POST/GET /api/teams/{team_id}/opportunities/{id}/notes`, `GET /api/teams/{team_id}/activity`.
...
*   **Opportunity Analysis & Reporting:** AI generates opportunity scores, market size estimates, and business model suggestions. A Consultant Studio offers a four-path validation system, and a report tracking system logs usage.
    ...
    - **Clone Success Payment Flow:** Non-authenticated users can purchase Clone Success analysis ($49) via Stripe Checkout. Free view shows blurred map, blurred location names (only match percentages visible), and locked demographics/competition/income/key factors. Payment flow: guest enters email, redirects to Stripe Checkout, returns with session_id for server-side verification before unlocking full details. Endpoints: `POST /api/v1/payments/create-clone-checkout`, `GET /api/v1/payments/verify-clone-checkout`.
    - **Performance Optimization:** Trade area analysis (Clone Success, Identify Location) uses async patterns via `asyncio.gather()` and `ThreadPoolExecutor` to reduce latency. Demographics fetch and map generation run in parallel. All blocking I/O (geocoding, competitor fetch, Census API calls, AI synthesis) are offloaded to a thread pool to keep the event loop non-blocking.
    - **AI Reliability System:** All AI calls implement 30-second timeouts via `asyncio.wait_for` to prevent hung requests. Structured error responses include `error_type` (ai_timeout, ai_error, ai_unavailable) for proper handling. Report generation wraps all operations in try/catch with automatic FAILED status on error. Admin cleanup endpoint (`POST /generated-reports/admin/cleanup-stuck`) marks reports stuck in GENERATING for >5 minutes as FAILED.
    - **Idea Validation Caching:** Database-backed IdeaValidationCache with 7-day TTL reduces redundant AI calls. Cache key uses SHA256 hash of idea + context. Race condition handling via IntegrityError catching for concurrent writes.
